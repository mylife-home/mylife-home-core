...
module.exports = class LocalRepository {
  ...
  uninstall(moduleName) {
    ...
    this._unloadModule(require.resolve(nodeModuleName));
    del.sync(path.join(rootDirectory, 'node_modules', nodeModuleName));
  }

  _unloadModule(id) {
    const module = require.cache[id];
    if(!module) { return; }
    module.children.forEach(this._unloadModule.bind(this));
