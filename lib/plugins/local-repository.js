...
- root
 - plugins
  - plugin-a
   - package.json (with main pointing to ./commit-ish, and module-specific meta)
...
async function npmInstall(directory, module) {
  // npm i --global-style --production --no-save --no-package-lock github:mylife-home/mylife-home-core-plugins-hw-absoluta
  return await common.utils.promise.fromCallback(done => child_process.exec(`npm i --global-style --production --no-save --no-package-lock ${module}`, {
    cwd: directory
...
module.exports = class LocalRepository {
  ...
  constructor() {
    ...
    for(const moduleName of moduleNames) {
      const { moduleMetadata } = loadJson(path.join(modulesDirectory, moduleName, 'package.json'));
  ...
  install(moduleMetadata, done) {
    return common.utils.promise.synchronize(async () => {
      ...
      // create package.json
      saveJson(path.join(moduleDirectory, 'package.json'), {
        main : `./${moduleMetadata.commit}`,
        moduleMetadata
      });
  ...
  _loadModule(metadata) {
    const moduleEntry = require(path.join(modulesDirectory, metadata.name));
    const plugins = [];
